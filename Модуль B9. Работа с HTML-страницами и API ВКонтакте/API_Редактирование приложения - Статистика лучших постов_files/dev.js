...
methodRun: function(hash, btn, paramsAdd) {
  ...
  if (isChecked('dev_const_edu')) {
    params['_edu'] = isChecked('dev_const_edu');
  }
  if (cur.verDD) {
  ...
  for(var i in params) {
    if (i.substr(0, 6) == 'param_') {
      ...
      url_params.push('params[' + i + ']=' + encodeURIComponent(params[i]));
    }
  }
...
checkUploadUrl: function(url) {
  ...
  if (url.match(/^((https?:\/\/)?)(?:[-a-z0-9\.]+\.)?(vkontakte\.ru|vk\.com|vk\.me|userapi\.com|vk-cdn\.net)\//ig)) {
    return true;
  }
  return false;
},
...
getResponseHelpers: function(response) {
  ...
  var uploadUrl = '';
  if (typeof response == 'string' && response.indexOf('/upload.php?') > 0) {
    uploadUrl = response;
  } else if (response.upload_url) {
    uploadUrl = response.upload_url;
  }

  // *.getUploadServer / *.marusiaAudioGetUploadLink
  if (uploadUrl && Dev.checkUploadUrl(uploadUrl)) {
    return '<div class="dev_upload_form">' +
      '<form id="dev_file_submit" action="' + clean(uploadUrl) + '" target="dev_upload_iframe" enctype="multipart/form-data" method="post">' +
      ...
      setTimeout(function () {
        if (wrap && wrap.classList) {
          wrap.classList.remove(
            'dev_main_featured_banners_anim',
            'dev_main_featured_banners_anim_' + nav,
            'dev_main_featured_banners_prev'
          );
        }
...
clickSpoiler: function(el, block_id, hash) {
  ...
  statlogsValueEvent('dev_section_spoiler_action', block_id, act, cur.page || 'title', cur.type || 'page');
},
...
stringifyOptions: function(options) {
  ...
  delete options.base_domain;
  delete options.login_domain;
...
initWidgetAppConstructor: function(options) {
  ...
  function updateCode() {
    var _options = {
      ...
      login_domain: options.login_domain || void 0
    };
    val(code_el, rs(options.tpl, {
      app_id: state.app_id,
      options: Dev.stringifyOptions(_options)
    }));
...
initWidgetAuthConstructor: function(options) {
  ...
  function updateCode() {
    ...
    if (state.mode == 2) {
      ...
      tpl = rs(tpl.replace('"%' + cb_key + '%"','%' + cb_key + '%'), rep);
    }
    val(code_el, tpl);
